...
def prune_by_percentile(percent, resample=False, reinit=False,**kwargs):
    '''
        Not currently working! Did not repair after trying to setup random pruning.
    '''
    ...
    for name, param in model.named_parameters():
        ...
        # Pick random percent of each layers' weight to set to 0. Don't resample.
        # naive, but quick implementation that just randomly turns them off until _ % are pruned?
        # would want to ensure that that amount is achieved after 1 pass? hmm... 
        # INSTEAD: figure out all indices and sample from them until _ % is not-pruned :D
        # could translate into setting all 0s, then sampling some to set to 1

        # We do not prune bias term
        if 'weight' in name:
            tensor = param.data.cpu().numpy()
            print(tensor.shape);continue
            tmp_shape = tensor.shape
            new_mask = np.flatten(np.zeros(tmp_shape))
...
def make_mask(model, percent=0):
    # percent dictates how much is masked out
    global step
    global mask
    step = 0
    for name, param in model.named_parameters(): 
