...
def main(args, ITE=0):
    ...
    # Data Loader
    ### This is the mean & std for just MNIST, yes? Hmm...
    ...
    assert images.size(-1) == 28, images.size(-1)
    std = torch.sqrt(var / (len(train_loader.dataset)*images.size(-1)*images.size(-2)))
